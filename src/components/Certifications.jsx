            ...
            {CERTIFICATES.map((certificate,index)=>(
                <div key={index} className="flex flex-wrap items-center justify-center mb-8">
                    <motion.div 
                        ...
                        <p className="mb-2 text-medium text-neutral-400">{certificate.date}</p>
                    </motion.div>
                    <motion.div 
                        whileInView={{opacity:1,x:0}}
                        initial={{opacity:0,x:-100}}
                        ...
                        <h6 className="mb-2 font-semibold">
                            {certificate.course } - {" "}
                            <span className="text-medium text-gray-500">{ certificate.platform}</span>
                        </h6>
                        <p className="mb-4 font-bold text-aqua-700">{certificate.description}</p>
                        {certificate.technology.map((technologies,index)=>(
